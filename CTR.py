...
class CTR:
    ...
    def encrypt(self, message):
        ...
        for i in range(len(message) // self._n):
            ...
            part_encrypted = self.xor_arrays(part_message, self._aes.encrypt([c for c in nonce]))
    ...
    def decrypt(self, message):
        ...
        for i in range(len(message) // self._n):
            part_message = message[i * self._n: (i + 1) * self._n].tolist()
            nonce = int(int.from_bytes(self._iv, byteorder='big') + i).to_bytes(16, 'big', signed=False)
            part_decrypted = self.xor_arrays(part_message, self._aes.encrypt([c for c in nonce]))
