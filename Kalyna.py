...
if __name__ == '__main__':
    ...
    message = [x for x in range(100000 // 8)]
    tests = [
        (128, 128, key22),
        (128, 256, key24),
        (512, 512, key88)
    ]
    for b_size, k_size, key in tests:
        ctx = init_context(b_size, k_size)
        key_expand(ctx, key)
        blocks = to_blocks(message, b_size)
        before = time.time()
        for block in blocks:
            encode(ctx, block)
        after = time.time()
        delta = after - before
        print('ENCRYPTION TIME ELAPSED {} sec for (Nb={}, Nk={}, Nr={})'.format(delta, ctx.Nb, ctx.Nk, ctx.Nr))
