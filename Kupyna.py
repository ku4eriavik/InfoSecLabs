...
class Kupyna:
    ...
    def __init__(self, n):
        self.mult_table = self._create_mult_table()
        self.n = n
        if 8 <= n <= 256:
            self.l = 512
            self.t = 10
    ...
    @staticmethod
    def _create_mult_table():
        MT = [[0 for j in range(256)] for i in range(256)]
        for x in range(256):
            for y in range(256):
                MT[x][y] = Kupyna.multiply_gf(x, y)
        return MT
        ...
        for col in range(self.c):
            for row in range(7, -1, -1):
                ...
                for b in range(7, -1, -1):
                    product ^= self.mult_table[state[b][col]][self.MDS[row][b]]
    ...
    def hash(self, message):
        if self.l == 512:
            ...
            hash_value = 1 << 1023
        input_states = self.padding(message)
...
def test():
    samples = [
        (
            ...
            int('08F4EE6F1BE6903B324C4E27990CB24EF69DD58DBE84813EE0A52F6631239875', 16),
            256
        ),
        (
            '000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F'
            ...
            int('0A9474E645A7D25E255E9E89FFF42EC7EB31349007059284F0B182E452BDA882', 16),
            256
        ),
        (
            '',
            int('CD5101D1CCDF0D1D1F4ADA56E888CD724CA1A0838A3521E7131D4FB78D0F5EB6', 16),
            256
        ),
        (
            '000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F'
    ...
    t_before = time()
    for s_input, s_output, s_size in samples:
        alg = Kupyna(s_size)
        hashed = alg.hash(s_input)
        assert hashed == s_output, 'Wrong!'
    t_after = time()
