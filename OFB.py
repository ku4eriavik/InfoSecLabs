...
class OFB:
    ...
    def encrypt(self, message):
        ...
        for i in range(len(message) // self._n):
            current = self._aes.encrypt(current)
            part_message = message[i * self._n: (i + 1) * self._n].tolist()
            part_encrypted = self.xor_arrays(part_message, current)
    ...
    def decrypt(self, message):
        ...
        for i in range(len(message) // self._n):
            current = self._aes.encrypt(current)
            part_message = message[i * self._n: (i + 1) * self._n].tolist()
            part_decrypted = self.xor_arrays(part_message, current)
